...
class ProfileScreen extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: Consumer<SimpleFirebaseUserProvider>(
        builder: (context, userProvider, _) {
          ...
          final displayName = user?.fullName ?? user?.email.split('@')[0] ?? 'Người dùng';
          final email = user?.email ?? '';

          // Khởi tạo controller với firebaseUid
          final ProfileController controller = Get.put(
            ProfileController(),
            tag: user?.id, // Sử dụng user ID làm tag để tránh conflict
          );

          return Obx(() {
            return RefreshIndicator(
              onRefresh: controller.loadApiData,
              color: const Color(0xFFd63384),
              child: SingleChildScrollView(
                physics: const AlwaysScrollableScrollPhysics(),
                child: Column(
                  children: [
                    _buildUserHeader(context, displayName, email, controller),
                    _buildUserLevel(context, controller),
                    _buildLearningSummary(context, controller),
                    _buildAchievementsOverview(context, controller),
  ...
  Widget _buildUserHeader(BuildContext context, String displayName, String email, ProfileController controller) {
    final apiUser = controller.userData;
  ...
  Widget _buildUserLevel(BuildContext context, ProfileController controller) {
    final progress = controller.progressData ?? {};
  ...
  // Các widget methods khác giữ nguyên logic, chỉ thay đổi cách lấy dữ liệu từ controller
  Widget _buildLearningSummary(BuildContext context, ProfileController controller) {
    final summary = controller.learningSummaryData ?? {};
  ...
  // Giữ nguyên tất cả các widget methods khác...
  // Chỉ cần thay thế _apiData bằng controller.apiData.value
  // và _isLoadingApiData bằng controller.isLoadingApiData.value
  ...
  Widget _buildAchievementsOverview(BuildContext context, ProfileController controller) {
    final achievements = controller.achievementsData ?? [];
  ...
  // Giữ nguyên tất cả các widget methods còn lại...
  // _buildAchievementItem, _buildRecentActivity, _buildAuthenticationUI, 
  // _buildQuickSettings, _buildSettingCard, _showLogoutDialog

  Widget _buildRecentActivity(ProfileController controller) {
    final activities = controller.recentActivitiesData ?? [];
  ...
  // Giữ nguyên các method widget khác...
  Widget _buildAchievementItem(BuildContext context, Map<String, dynamic> achievement) {
    ...
    IconData getIconData(String iconStr) {
      switch (iconStr.toLowerCase()) {
        ...
        case 'start': case 'star': return Icons.star_rounded;
        case 'work': case 'works': return Icons.work_rounded;
        case 'pace': return Icons.speed_rounded;
        case 'trophy': return Icons.emoji_events_rounded;
        case 'book': return Icons.menu_book_rounded;
        case 'fire': case 'flame': return Icons.local_fire_department_rounded;
  ...
  Widget _buildActivityItem(Map<String, dynamic> activity) {
    ...
    IconData getIcon(String type) {
      switch (type) {
        ...
        case 'review': return Icons.replay;
        case 'learn': return Icons.library_books;
    ...
    Color getColor(String type) {
      switch (type) {
        case 'quiz': return const Color(0xFFae3ec9);
        case 'review': return const Color(0xFFf06595);
        case 'learn': return const Color(0xFFd63384);
        case 'achievement': return const Color(0xFFa61e4d);
