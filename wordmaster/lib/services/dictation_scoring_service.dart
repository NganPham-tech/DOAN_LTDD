...
class DictationScoringService {
  ...
  static DictationResult scoreText({
    ...
    // Split into words
    final userWords = normalizedUser.split(' ').where((w) => w.isNotEmpty).toList();
    final correctWords = normalizedCorrect.split(' ').where((w) => w.isNotEmpty).toList();
  ...
  static List<WordComparison> _compareWords(List<String> userWords, List<String> correctWords) {
    final comparisons = <WordComparison>[];
    final maxLength = userWords.length > correctWords.length ? userWords.length : correctWords.length;
  ...
  static int _levenshteinDistance(String s1, String s2) {
    ...
    // Create matrix
    final matrix = List.generate(len1 + 1, (i) => List<int>.filled(len2 + 1, 0));
